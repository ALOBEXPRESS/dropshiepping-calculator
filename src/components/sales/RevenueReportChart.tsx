...
export const RevenueReportChart: React.FC<RevenueReportChartProps> = ({ organizationId, refreshTrigger, onOrderDeleted, onOrderUpdated, period: externalPeriod, onPeriodChange, onRegisterOpenOrder, onRegisterOpenAff, onAffDeleted }) => {
  ...
  useEffect(() => {
    const fetchMarketingCosts = async () => {
      ...
      const { data: cocRows } = await supabase
        .from('campaign_order_costs')
        .select('order_id, marketing_cost, campaign_id')
        .in('order_id', allOrderIds);
      const manualOverrideIds = new Set<string>(); // orders explicitly set by user
      for (const row of (cocRows ?? []) as Array<{ order_id: string; marketing_cost: number; campaign_id: string | null }>) {
        const cost = Number(row.marketing_cost ?? 0);
        costMap[`order:${row.order_id}`] = cost;
        manualOverrideIds.add(row.order_id);
        // Todos os campaign_order_costs descontam do lucro do pedido
        // (existência no coc significa custo explicitamente atribuído ao pedido)
      ...
      // STEP 2: campaign_products for orders NOT already overridden by user
      const remainingIds = allOrderIds.filter(id => !manualOverrideIds.has(id));
      if (remainingIds.length > 0) {
        const { data: cpRows } = await supabase
          .from('campaign_products')
          .select('campaign_id, linked_order_id, marketing_cost_override')
          .in('linked_order_id', remainingIds)
        ...
        for (const row of (cpRows ?? []) as Array<{ campaign_id: string | null; linked_order_id: string; marketing_cost_override: number }>) {
          ...
          if (row.campaign_id) {
            if (!seenCampaigns.has(row.campaign_id)) {
              ...
              // campaign_products without coc entry = group cost, NOT per-order deduction
            } else {
