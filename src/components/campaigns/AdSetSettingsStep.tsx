...
const Accordion: React.FC<{
  ...
  return (
    <div className="rounded-lg border border-zinc-800 overflow-hidden">
      ...
      {open && <div className="px-4 py-4 space-y-4 bg-zinc-950/40">{children}</div>}
    </div>
  );
};
...
export const AdSetSettingsStep: React.FC<AdSetSettingsStepProps> = ({
  ...
  // Raw local state for cost input
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ── Campanha ─────────────────────────────────────────────── */}
      <Accordion title="Campanha" defaultOpen>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <Input
                  ...
                  onChange={(e) => onChange('audience_behavior', e.target.value.replace(/[^0-9,.]/g, '') || null)}
      ...
      {/* ── Configurações de Consideração ────────────────────────── */}
      {isConsideration && (
        <Accordion title="Configurações de Consideração" defaultOpen>
          <div className="space-y-1.5">
      ...
      <Accordion title="Posicionamento">
        <Label className="text-zinc-300 text-sm block mb-2">Local de Exibição</Label>
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            const isSelected = data.placement === opt.value;
            return (
              <button
                key={opt.value}
                type="button"
      ...
      <Accordion title="Nível de Anúncio">
        ...
        <div className="space-y-1.5">
          ...
          <Input
            type="url"
            placeholder="https://"
            value={adSetData.ad_redirect_url ?? ''}
            onChange={(e) => onChange('ad_redirect_url' as keyof CampaignFormPayload['adSet'], e.target.value || null)}
            className="bg-zinc-900 border-zinc-700 text-white placeholder:text-zinc-500 focus:border-orange-500"
          />
        </div>
      </Accordion>
