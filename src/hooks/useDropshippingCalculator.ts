...
export const useDropshippingCalculator = () => {
  ...
  const [productFilters, setProductFilters] = useState<{
    ...
    if (typeof window === 'undefined' || !('localStorage' in window)) {
      return {
        ...
        affiliateFilter: 'all', categoryFilter: 'all', minProfit: '', maxProfit: '', minPrice: '', maxPrice: '',
        promoFilter: 'all', promoTypeFilter: 'all'
      };
    }
    try {
      ...
      if (saved) {
        ...
        return { affiliateFilter: 'all', categoryFilter: 'all', minProfit: '', maxProfit: '', minPrice: '', maxPrice: '', promoFilter: 'all', promoTypeFilter: 'all', ...parsed };
      }
    } catch (error) {
  ...
  const calculations = useMemo<CalculationResult | null>(() => {
    ...
    return calculateMetrics(
        ...
        parseCurrency(tiktokPromoProductValue) || 0,
    ...
    tiktokAdsEnabled, tiktokDailyBudget, tiktokAdsSalesQuantity, tiktokCPA, tiktokCPM, tiktokCTR, tiktokCVR, tiktokSfpEnabled, influencers, affiliates, tiktokPromoProductValue, tiktokPromoProductType]);
